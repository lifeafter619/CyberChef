...
class OutputWaiter {
    ...
    async eolChange(eol, manual=false) {
        ...
        if (this.eolState === 1) {
            // Alert
            this.app.alert(`已检测到输出的行结束符，并已更改为 ${eolCodeToName[eol]}`, 5000);
        }
    ...
    async chrEncChange(chrEncVal, manual=false) {
        ...
        if (this.encodingState > 1) {
            ...
            // Alert
            this.app.alert(`已检测到输出字符编码，并已更改为 ${CHR_ENC_SIMPLE_REVERSE_LOOKUP[chrEncVal] || "原始字节"}`, 5000);
        }
    }
    ...
    addOutput(inputNum, changeTab = true) {
        ...
        const newOutput = {
            data: null,
            inputNum: inputNum,
            statusMessage: `输入 ${inputNum} 尚未烘焙。`,
            error: null,
            status: "inactive",
            bakeId: -1,
            progress: false,
    ...
    async downloadFile() {
        ...
        if (dish === null) {
            this.app.alert("未找到可下载的输出数据。该输出是否已烘焙？", 3000);
            return;
        }
        ...
        const fileName = window.prompt("请输入文件名：", `下载${ext}`);
    ...
    async downloadAllFiles() {
        ...
        let fileName = window.prompt("请输入文件名：", "下载.zip");
        ...
        let fileExt = window.prompt("请输入文件扩展名，或留空以自动检测。", "");
    ...
    terminateZipWorker() {
        ...
        downloadButton.title = "将所有输出保存为 ZIP 文件";
        downloadButton.setAttribute("data-original-title", "将所有输出保存为 ZIP 文件");
    ...
    goToTab() {
        ...
        let tabNum = window.prompt(`请输入标签页编号（${min} - ${max}）：`, this.manager.tabs.getActiveTab("output").toString());
    ...
    async copyClick() {
        const dish = this.getOutputDish(this.manager.tabs.getActiveTab("output"));
        if (dish === null) {
            this.app.alert("未找到可复制的数据。该输出是否已烘焙？", 3000);
            return;
        }
        ...
        navigator.clipboard.writeText(output).then(function() {
            self.app.alert("已成功复制原始输出。", 2000);
        }, function(err) {
            self.app.alert("抱歉，无法复制输出。", 3000);
        });
    }
